...
-- LETRA A -Popular a tabela DEPARTAMENTO, inserindo no mínimo sete departamentos.
...
-- LETRA B -  Popular a tabela FUNCIONARIO, inserindo no mínimo 3 funcionários para cada departamento criado.
...
-- LETRA C - Popular todos os ESTADOS do Brasil e associe no mínimo três cidades para cada Estado. Para cada cidade, associe no mínimo três bairros, e para cada bairro, associe três endereços. Utilize nomes significativos e coerentes, de acordo com a base do Correio. Para contribuir, segue uma sugestão de link de endereços
...
-- LETRA D - Por fim, cadastre na tabela DE ENDERECO FUNCIONARIO todos os funcionários com no mínimo um endereço. Escolha vários Estados do Brasil, ou seja, um endereço para cada localidade. Utilize nomes significativos e relevantes.
...
-- LETRA F - Cadastre um novo cliente que já tenha um mesmo nome de login já criado. (*Exiba a instrução SQL executada para realizar a tarefa e apresente o resultado dessa execução.) Foi possível incluir esse novo cliente? Explique.
...
-- LETRA H  (CATEGORIA VIDEO) - Cadastre as seguintes categorias para os vídeos: Instalação do Produto; Uso no Cotidiano; Comercial com Personalidade; entre outros.
...
-- LETRA I -  Cadastre 20 produtos e associe às categorias adequadas ao produto
...
-- LETRA J - Cadastre três vídeos de produtos e associe às categorias adequadas a cada vídeo do produto. Na coluna ds_path_video_prod, assuma um caminho-padrão em que os vídeos podem estar localizados.
...
-- LETRA K - Por fim, cadastre duas visualizações de vídeos de produtos e associe a um cliente a seu critério.
...
-- LETRA N - Selecione um específico funcionário e atualize seu cargo e também aplique 12% de aumento de salário.   
...
-- LETRA P - Atualize a data de nascimento de um cliente pessoa física. Defina a nova data como sendo 18/5/2002.

UPDATE t_mc_cli_fisica
    SET dt_nascimento = TO_DATE('18/05/2002','DD/MM/YYYY')
    WHERE nr_cliente = 1; 
    
-- LETRA Q - Desative um funcionário colocando o status como I(nativo) e também a data de desligamento como sendo a data de hoje (sysdate).
...
-- LETRA R SAC -  Atualize a descrição detalhada de retorno do SAC feito pelo funcionário. Insira um conteúdo significativo. Não se esqueça de atualizar também a data e hora de atendimento, bem como acrescendo o número total de horas do atendimento SAC.
...
-- LETRA S - Selecione um endereço de cliente, coloque o status como I(nativo) e preencha a data de término como sendo a data de ontem. Utilize a função to_date para registrar esse novo valor da data.
...
-- LETRA T - Tente eliminar um Estado que tenha uma cidade cadastrada. Isso foi possível? Justifique o motivo.
...
-- LETRA U - elecione um produto e tente atualizar seu status com o valor X. Isso foi possível? Justifique o motivo.
...
-- LETRA V - Confirme todas as transações pendentes.
